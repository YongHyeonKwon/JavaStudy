			...
			if (level == 1) {
				...
				if (transport == 2) {
					...
					while (true) {
						if (pickUp == 1) {
							...
							fc.spendTime(10);
							fc.timeAndStress();
							System.out.println("동생을 태워주며 10여분이 소요되었다.");
							fc.timeAndStress();
							break;
						} else if (pickUp == 2) {
				...
				while(true) {
					...
					if (num == 1) {
						...
						fc.spendTime(50);
						fc.getStress(100);
						System.out.println("[스트레스 지수 100↑ , 시간 30분 소요]");
						fc.timeAndStress();
...
//					break;
                ...
                while(true) {
	                ...
	                if (num == 1) {
	                   ...
	                   fc.spendTime(120);
	                   fc.getStress(100);
	                   System.out.println("[스트레스 지수 50↑ , 시간 120분 소요]");
	                   fc.timeAndStress();
	                   break;
	                }else
	                	fc.infoNum();
...
//					break;
                ...
                while(true) {
	                ...
	                if (num == 1) {
	                   ...
	                   fc.spendTime(60);
	                   System.out.println("[스트레스 지수 0↑ , 시간 60분 소요]");
	                   fc.timeAndStress();
...
//					break;
                ...
                while(true) {
	                ...
	                num = sc.nextInt();
	                if (num == 1) {
	                	System.out.println("(요즘엔 카메라가 깔려있답니다..)[주차단속에 걸렸습니다.]");
	                   fc.getStress(+40);
	                   fc.spendTime(5);
	                   fc.timeAndStress();
...
//					break;
				...
				break;
					...
					while(true) {
						...
						System.out.println("주인공 : 아침이라 몸도 피곤한데 이럴 때는 몸을 깨워줘야해! 카페인 충전에는 커피가 최고지!");
					...
					if(num ==1) {
						System.out.println("주인공 : 나는 커피로 하루를 시작하는 뉴요커~ 요걸 마시면 기분이 조크든요~");
						fc.timeAndStress();
					...
					System.out.print("버스기사 : 학생 음료 들고 버스 못타. 버리던지 마시던지 해");
					
					fc.timeAndStress();
					
					System.out.println("[버리던지 마셔버릴지 어떡할것인가]");
					while(true) {
...
//						break;
					...
					// 시련 5 (방귀)
		               ...
		               System.out.println("[남은 시간은 10분 뛰어가면 15분다");
